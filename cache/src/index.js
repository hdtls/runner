...
const CACHE_PATH = path.join(os.homedir(), 'Library/Caches/com.nektos.Actions/actions');
await fs.mkdir(path.join(CACHE_PATH, 'fsCachedData'))
const PART_PREFIX = '/tmp/.cache_';
const PART_EXT = '.part';
const PART_META_EXT = '.meta';

// DB Setup
const db = new sqlite3(path.join(os.homedir(), 'Library/Caches/com.nektos.Actions/fsCachedData/bolt.db'));
try {
    db.prepare("" +
        "CREATE TABLE caches (" +
...
const authmiddleware = (req, res, next) => {
    if (
      req.get("Authorization") !== `Bearer ${process.env.ACTIONS_RUNTIME_TOKEN}`
    ) {
